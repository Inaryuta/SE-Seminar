document.addEventListener("DOMContentLoaded", async () => {
    ...
    if (isEditMode) {
        ...
        document.querySelector("h2").textContent = "Edit Exercise Session";
        const saveBtn = document.getElementById("saveSessionBtn");
        saveBtn.textContent = "Update Session";

        try {
            const res = await fetch(`http://localhost:8000/routines/${routineId}/sessions/${sessionId}`);
            const data = await res.json();

            // Rellenar los campos principales
            document.getElementById("date").value = data.date;
            document.getElementById("duration").value = data.duration_minutes;
            document.getElementById("intensity").value = data.session_intensity;
            document.getElementById("calories").value = data.calories_burned;
            document.getElementById("routineName").value = data.routine_name;

            // Rellenar ejercicios (asumiendo que vienen como data.exercises)
            for (let ex of data.exercises) {
                ...
                // Espera a que los ejercicios se carguen para agregar opciones
                ...
                // Establecer selección y valores
                select.value = ex.exercise_id;
                select.dispatchEvent(new Event("change"));

                block.querySelector(".sets").value = ex.sets || "";
                block.querySelector(".reps").value = ex.reps || "";
                block.querySelector(".weight").value = ex.weight || "";
                block.querySelector(".distance").value = ex.distance_km || "";
    ...
    saveSessionBtn.addEventListener("click", async () => {
        ...
        let routineName = document.getElementById("routineName").value.trim();
        if (!routineName) {
            routineName = "Routine for " + new Date().toLocaleDateString();
        }
